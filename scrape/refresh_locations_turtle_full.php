...
refresh_locations_turtle_all();
function refresh_locations_turtle_all()
{
    $animal_manager = Animal_Manager::Instance();
    $animalsArray = $animal_manager->getAnimalsFromOriginType(1);
    ...
    foreach ($animalsArray as $animalItem) {
        if ($loops <= $maxloops && $loops >= $startloops) {
            refresh_locations_animal($animalItem);
        }
